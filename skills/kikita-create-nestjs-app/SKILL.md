...
2. **Empty or near-empty directory** (no `package.json`/`nest-cli.json`, nothing but maybe a
   `.git/`/README the user just created) → fresh init. Continue with section 1 below, then
   `plan.md`.
3. **Existing project, no scaffold record** (`package.json`/`nest-cli.json` present, no
   `.agents/.kikita-scaffold.json`) → this is someone else's (or an older, pre-this-skill)
   NestJS project. Do **not** run `plan.md` — it assumes an empty directory and will try to
   `nest new` over a real project. Follow `adopt.md` instead: it generates the `.agents/`
   tree against what's actually there, inferring questionnaire answers from the existing
   config rather than scaffolding new tooling, and asks before changing anything that
   already exists.
...
1. **Application type**: REST API, bot, or both in the same app?
   - If bot (or both): which platform? Telegram (`nestjs-telegraf`) or Discord — and for
     Discord specifically, `necord` is the recommended default (least glue code to satisfy the
     generic event→handler→service pattern via native Nest DI/guards/pipes) but not mandatory:
     if the user wants raw `discord.js` instead (existing team expertise, a `discord.js` feature
     `necord` doesn't wrap yet), that's a legitimate choice — treat it the same as "another
     platform" below, adapting the generic pattern by hand instead of getting it for free.
...
8. **i18n**: does this project need multiple locales — translated REST error/validation
   messages and/or bot replies in more than one language? If yes, no further sub-question —
   `nestjs-i18n` is the fixed library; locale resolution differs by transport (REST:
   `Accept-Language` header; bot: the platform's own per-user locale field, never
   `Accept-Language`) and both are covered in `core/i18n.md`. If no, skip — a project that will
   only ever run in one language doesn't need this.
9. **JSDoc/TSDoc on public API**: enforce mandatory doc comments on every exported symbol, or
   skip it? Drives whether `.agents/agent-surface.md` is generated (recommended default: yes).
10. **Git policy**: may the agent commit and push without asking each time, or must every
    commit/push be confirmed?
11. **Package manager**: npm, pnpm, or yarn? Default recommendation: pnpm.
12. **Git remote**: does the user already have a repo URL to push to? If yes, record it —
...
- **Inclusion gates** — application type, bot platform, tests, auth, queue, cache, storage,
  ...
  whole file (or a `<!-- SCAFFOLD -->`-marked block inside one) is copied at all. A "no" answer
  means the file/block is deleted, not filled with an empty string. Gated files must still be
  linked from `AGENTS.md` / the relevant README when kept, and their links removed when skipped.
...
- `.agents/README.md` is a flat index of everything under `.agents/` — keep it in sync whenever a
  conditional file (auth, queue, cache, storage, messaging, i18n, agent-surface) is added or
  skipped.
- Topics that are genuinely one short doc stay flat in `.agents/*.md` (workflow, git-policy,
  documentation, testing-and-quality, file-change-review, agent-surface, refactoring, progress).
...
- Every generated project's source-file create/change/move runs the per-file gate in
  `.agents/file-change-review.md`, including the ownership/path inventory, declaration extraction,
  decomposition thresholds, test decision, and comment review. The gate is mandatory after each
  file, not a single final glance over the feature.
