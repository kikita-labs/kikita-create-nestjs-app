...
6. If a Prisma schema change was made, run the migration (`npx prisma migrate dev --name
...
7. Update the matching `.agents/` doc in the same change if you added, removed, or changed a
...
8. Run lint, format check, and tests (whichever are configured) before committing. See
   `.agents/testing-and-quality.md`.
9. Before committing, run `git diff --check` and scan the diff for Cyrillic or mojibake —
...
10. Commit following `.agents/git-policy.md`.
